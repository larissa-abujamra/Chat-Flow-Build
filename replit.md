...
- Research node conventions (id-based, no schema/openapi change — survive UI edits because node ids are preserved; each degrades to just the node's question on failure or when OpenRouter is absent). Both implemented as helpers in `chat.ts` invoked when the chat advances INTO the matching `target.id`:
  - `companyResearch` (`RESEARCH_NODE_ID`, `researchCompany()`): Sonar Pro Search with the onboarding transcript fetches preliminary public company info and PREPENDS a short, **statements-only** summary. The system prompt forbids questions/focus/next-step suggestions, and `stripTrailingQuestions()` removes any trailing question paragraphs the model still adds — so this step never asks the user anything (it previously leaked a "what are you focused on?" question).

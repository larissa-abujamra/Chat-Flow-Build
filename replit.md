...
- Onboarding sequence (ALL responses are REAL — no mocked/wireframe data in the data-collection or research steps). Order: `q_nome` (start; multi-bubble intro via `---`, last bubble invites the user to add teammates by email) → `q_negocio` ("Agora sim, poderia me informar o nome do seu negócio?") → `q_cnpj` (asks for the CNPJ) → `confirmaDados` (BrasilAPI enrichment) → `catalogo` (research products) → `q_site` (asks for the website) → `confirmaInstagram` (Instagram handle extracted from the site HTML) → `igConectado` ("Pronto, conectado!") → the tone-of-voice / tour tail (`tomGerado`/`tomManual`/`emojis`/… unchanged). Branch wiring: `confirmaDados` (Sim → `catalogo`, Não → back to `q_negocio`); `catalogo` (Sim → `q_site`, "Falta coisa" → `faltaCoisa` → `q_site`); `confirmaInstagram` (ig-ok → `igConectado`, ig-nao "não tem / não quer conectar" → `tomManual`). The open-ended nodes (`q_nome`, `q_negocio`, `q_cnpj`, `q_site`) each carry ONE permissive branch ("user provided X") so any answer advances. Removed nodes vs. prior versions: `confirmaNegocio` (superseded by `confirmaDados`), `instagram` + `igUsername` (superseded by `q_site` + `confirmaInstagram`), and the older `q_instagram`/`q_setor`/`companyResearch`/`carroChefe`.

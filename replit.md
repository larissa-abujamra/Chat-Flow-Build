...
- Onboarding sequence (ALL responses are REAL — no mocked/wireframe data in the data-collection or research steps). Order: `q_nome` (start; multi-bubble intro via `---`, last bubble invites the user to add teammates by email) → `q_negocio` ("Agora sim, poderia me informar o nome do seu negócio?") → `q_site` (asks for the website) → `confirmaInstagram` (Instagram handle extracted from the site HTML) → `igConectado` ("Pronto, conectado!") → `q_cnpj` (asks for the CNPJ) → `confirmaDados` (BrasilAPI enrichment) → `catalogo` (research products) → the tone-of-voice / tour tail (`tomGerado`/`tomManual`/`emojis`/… unchanged). **The website is asked EARLY (right after the business name) on purpose: by the time the CNPJ-enrichment and catalog-research steps run, the site URL is already in the transcript, so the Sonar catalog lookup can use the real site and product confirmation happens late (lower error). `confirmaInstagram` MUST still immediately follow `q_site` because `researchInstagramFromSite()` reads the URL from `lastUser.content`.** Branch wiring: `confirmaInstagram` (ig-ok → `igConectado`, ig-nao "não tem / não quer conectar" → `q_cnpj` so data collection continues); `igConectado` → `q_cnpj`; `confirmaDados` (Sim → `catalogo`, Não → back to `q_negocio`); `catalogo` (Sim → `tomGerado`, "Falta coisa" → `faltaCoisa` → `tomGerado`). Note: because the IG step now precedes the tone tail by several nodes, a user with no Instagram still reaches the auto-tone `tomGerado` (which offers a "Quero ajustar" → `tomManual` escape) rather than jumping straight to manual tone as before. The open-ended nodes (`q_nome`, `q_negocio`, `q_cnpj`, `q_site`) each carry ONE permissive branch ("user provided X") so any answer advances. Removed nodes vs. prior versions: `confirmaNegocio` (superseded by `confirmaDados`), `instagram` + `igUsername` (superseded by `q_site` + `confirmaInstagram`), and the older `q_instagram`/`q_setor`/`companyResearch`/`carroChefe`.
- Research node conventions (id-based, no schema/openapi change — survive UI edits because node ids are preserved; each degrades to just the node's question on failure or when OpenRouter is absent). Implemented as helpers in `chat.ts` invoked when the chat advances INTO the matching `target.id`:
  - `confirmaDados` (`CNPJ_NODE_ID`, `researchCnpj()`): looks the CNPJ the user just typed up on the free, key-less BrasilAPI (`https://brasilapi.com.br/api/cnpj/v1/{digits}`, non-digits stripped; must be 14 digits) and PREPENDS a short factual summary (📛 nome fantasia/razão, 🏷️ setor from `cnae_fiscal_descricao`, 📍 cidade `municipio - uf`) before the confirmation question. **BrasilAPI 403s the default Node `user-agent: node`, so the request MUST send a browser-like `user-agent` header.** Invalid/absent CNPJ or any API failure degrades to `researchBusiness()` (Sonar name-lookup), which itself degrades to just the node question.
  - `confirmaInstagram` (`SITE_INSTAGRAM_NODE_ID`, `researchInstagramFromSite()`): the user gives their website at `q_site`; the server fetches the site HTML (browser-like UA, follows redirects, requires an HTML/XML content-type, reads the full body up to a 3MB cap — NOT a head slice, because footer social links sit near the END) and extracts the first real Instagram handle via `extractInstagramHandle()` (regex tolerates JSON-escaped `instagram.com\/handle`; skips reserved/content segments `p`/`reel`/`reels`/`explore`/`stories`/… via `IG_RESERVED`). On success it PREPENDS "Achei seu Instagram pelo site: @handle 🎉 …"; on any failure (bad/no URL, unreachable, 403, no handle) it degrades to just asking for the handle. The user-supplied URL is sanitized by `normalizeSiteUrl()` (http/https only; rejects localhost/private/link-local/metadata hosts — a basic, hostname-only SSRF guard, no DNS resolution).

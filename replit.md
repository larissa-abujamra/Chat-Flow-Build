...
- Onboarding lead-in: the flow starts with open-ended data-collection questions (business name, CNPJ, site, Instagram, sector). Each is a normal node with ONE permissive branch ("user provided X") so any answer advances. After those, a node with the reserved id `catalogo` runs a web-research step.
- Research node conventions (id-based, no schema/openapi change — survive UI edits because node ids are preserved; each degrades to just the node's question on failure or when OpenRouter is absent). Both implemented as helpers in `chat.ts` invoked when the chat advances INTO the matching `target.id`:
  - `catalogo` (`CATALOG_NODE_ID`, `researchCatalog()`): Sonar Pro Search of the company's REAL site + Instagram (from the transcript) lists up to 6 actual products it finds (price or "preço a confirmar"), PREPENDED before the node question. If none are found (model returns `NO_PRODUCTS`/empty) it says so honestly instead of inventing a catalog. The `catalogo` node question is just "Esse é o seu catálogo?" — the product list is built live by the server, so the node text carries no hardcoded/wireframe product placeholder.
  - There used to be a separate `companyResearch` node that PREPENDED a public-company summary before the catalog step. It was removed as redundant (the catalog/product message is the only relevant one). Older flow snapshots still embed leftover wireframe text in the `catalogo` node question ("(mostro um card com 4 itens…)" parenthetical and an "Achei esses produtos seus." sentence). `sanitizeCatalogQuestion()` strips both; `displayQuestion(node)` applies it to ANY user-facing emission of a node's question (start, no-input, no-match re-ask, advance) when `node.id === "catalogo"`, so wireframe text never leaks regardless of which path emits it.
